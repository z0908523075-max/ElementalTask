...
class PartOfSpeechTask(BaseTask):
    ...
    def evaluate(self, predictions: List[str], split: str = "test", **kwargs) -> Dict[str, float]:
        """評估預測 with exact-match 準確率 (不區分大小寫, 去除首尾空白)."""
...
def create_pos_task(
    ...
    # --- 預設: 15 範例 (from the conversation) ---
    ...
    # Prepare 資料: ensure "prompt" is present and add a "word" alias for template compatibility if needed
    raw = deepcopy(dataset if dataset is not None else default_examples)
    prepared: List[Dict[str, Any]] = []
    for ex in raw:
        ex = {**ex}
        # alias to 支援 templates that might use {word}
        ex.setdefault("word", ex.get("target_word", ""))
        # derive 提示 if not provided
        ex.setdefault("prompt", prompt_format.format(**ex))
        # BaseTask expects "input" and "output" columns for 評估 by 預設—map them here:
        ex["input"] = ex["prompt"]
        ex["output"] = ex["answer"]
        prepared.append(ex)
    ...
    config = TaskConfig(
        ...
        input_column="input",     # uses constructed 提示 字串
        output_column="output",   # expected POS label
        prompt_template="{input}",  # pass-through: we've already 格式化 the 提示
