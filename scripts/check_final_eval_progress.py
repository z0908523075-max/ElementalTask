#!/usr/bin/env python3
"""Checkfinal-eval task completion status across model.
...
- done:  metrics file >= expected checkpoint count
- partial: 0 < metrics file < expected
- missing: metrics file == 0

It also prints missing task indices and a ready-to-run sbatch command:
  sbatch --array=<missing_indices> <launcher_script>

Optionally, it annotates task currently RUNNING/PENDING in SLURM queue.
"""
...
def task_aliases(task: str) -> List[str]:
    """Returnscanonical and known on-disk aliases for a task name."""
...
def count_outputs(output_base: Path, task: str) -> int:
    ...
    Prefer metrics file when available, but fall back to detailed outputs for
    task that only emit detailed JSONL file.
    """
    best = 0
...
def queue_index_map() -> Dict[str, Dict[int, str]]:
    """Returns{job_name: {task_idx: running|pending}} from squeue.
